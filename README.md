...
2. Натискає **"🔄 Синхронізувати"** — бот витягує слова з таблиці і зберігає у Postgres (diff: додає нові, оновлює змінені переклади/транскрипції, видаляє ті що зникли).
...
На сервері, у папці проєкту, строго в такому порядку:

```bash
git pull
npm install                  # postinstall перегенерує Prisma client (обов'язково, якщо мінялась schema.prisma)
npx prisma migrate deploy    # ДО рестарту: новий код може вже читати нові колонки
npm run build
pm2 restart <назва процесу>  # подивитись назву: pm2 list
```

Після деплою:
- Якщо в `changelog.ts` є запис з `announce: true` — зайти в бота як адмін і розіслати через «📢 Анонсувати».
- Якщо змінилась логіка читання таблиці — юзерам треба натиснути «🔄 Синхронізувати», щоб дані підтягнулись.
